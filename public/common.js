...
browser.storage.sync.get().then(data => {
  ...
  if (data.version === 1) {
    ...
    if (!Array.isArray(data.blocklist) && typeof data.blocklist !== 'string') { browser.storage.sync.set({ blocklist: '[]' }) }
    if (typeof data.disabled !== 'boolean' && typeof data.disabled !== 'string') { browser.storage.sync.set({ disabled: 'false' }) }
  }
})
